"""T019: quantitative decision-conflict benchmark on PhysioNet Challenge 2012.
...
def main():
    ...
    for s,P in enumerate(stage_probs[:-1],1):
        ...
        for name,val in [("disagreement",disagree),("conflict_mass",conflict_mass),("vote_margin",vote_margin),("low_confidence",1-conf),("entropy",entropy),("margin_uncertainty",margin),("prob_sd",sd)]:
            metrics[name]=scores(rev,val)
        summary.append(metrics)
        for j,k in enumerate(te):
            records.append({"stage":s,"case_index":int(k),"reversal":int(rev[j]),"disagreement":int(disagree[j]),"conflict_mass":float(conflict_mass[j]),"vote_margin":float(vote_margin[j]),"confidence":float(conf[j]),"entropy":float(entropy[j]),"margin_uncertainty":float(margin[j]),"prob_sd":float(sd[j])})
    pd.DataFrame(records).to_csv("T019_cases.csv",index=False)
    with open("T019_summary.json","w") as f: json.dump(summary,f,indent=2)
